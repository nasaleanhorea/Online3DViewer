...
export function ConvertModelToThreeObject(
  model,
  conversionParams,
  conversionOutput,
  callbacks,
  ...
  function CreateThreeTriangleMesh(meshInstance, materialHandler) {
    ...
    triangleIndices.sort((leftTriangleIndex, rightTriangleIndex) => {
      let leftTriangle = mesh.GetTriangle(leftTriangleIndex);
      let rightTriangle = mesh.GetTriangle(rightTriangleIndex);
    ...
    for (let triangleIndex of triangleIndices) {
      ...
      let firstVertex = mesh.GetVertex(triangle.v0);
      let secondVertex = mesh.GetVertex(triangle.v1);
    ...
    let threeMesh = new THREE.Mesh(
      threeGeometry,
      meshMaterialHandler.meshThreeMaterials,
    );
  ...
  function CreateThreeLineMesh(meshInstance, materialHandler) {
    ...
    let threeLine = new THREE.LineSegments(
      threeGeometry,
      meshMaterialHandler.meshThreeMaterials,
    );
    threeLine.userData = {
  ...
  let materialHandler = new ThreeMaterialHandler(
    model,
    stateHandler,
    conversionParams,
    conversionOutput,
